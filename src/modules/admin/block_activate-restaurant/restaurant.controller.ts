...
export const restaurantAdminController = {
  ...
  async getActiveRestaurants(req: Request, res: Response) {
    try {
      const restaurants = await restaurantAdminService.getActiveRestaurants(getMallScope(req));
  ...
  async getBlockedRestaurants(req: Request, res: Response) {
    try {
      const restaurants = await restaurantAdminService.getBlockedRestaurants(getMallScope(req));
      return res.json(restaurants);
    } catch (err: any) {
  ...
  async searchRestaurants(req: Request, res: Response) {
    try {
      ...
      const restaurants = await restaurantAdminService.searchRestaurants(search, getMallScope(req));
      return res.json({ restaurants, total: restaurants.length });
    } catch (err: any) {
      return res.status(500).json({ message: 'Failed to search restaurants', error: err.message });
    }
  },
};
